...
# skill zwendorowany w repo: szablon leży w .claude/skills/, nie w ~/.claude/skills/
SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null)/.claude/skills/threads-carousel"
[ -d "$SKILL_DIR/template" ] || SKILL_DIR=~/.claude/skills/threads-carousel
rsync -a --exclude=node_modules "$SKILL_DIR/template/" "$WORK_DIR/"
ln -s "$SKILL_DIR/template/node_modules" "$WORK_DIR/node_modules"
cd "$WORK_DIR"
```
