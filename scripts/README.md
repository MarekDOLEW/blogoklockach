...
|---|---|---|
| `feedy-lego.py` | Łowca, codziennie | Wyciąga z feedów sklepowych wyłącznie oferty LEGO (~4 MB zamiast ~630 MB). Dodatkowo: **nadpisuje adresy kart w `redirects.json`** (ME, PK, Allegro) świeżymi z feedu — wpisów nie kasuje, przerywa gdyby liczba zmalała; uruchamia importery feedów TD oznaczonych `"odswiezanie": "codziennie"` oraz `historia-cen.mjs`. `--tylko`, `--tylko-td`, `--bez-td` |
...
| `kontrola-linkow.mjs` | Kontroler, co tydzień (poniedziałek) | Losowa próba linków z `redirects.json`: wyciąga z linku **adres docelowy sklepu** i sprawdza jego kod HTTP. **Nigdy nie odpytuje linku trackingowego** — to byłby sztucznie nabity klik w sieci afiliacyjnej. Martwe karty (404/410) idą mailem (`--zadanie linki`); Allegro, Media Expert i LEGO.com odrzucają ruch serwerowy (403) i trafiają do kolumny „blokada sklepu", nie do „żywe". `--ile N`, `--sklep x`, `--sucho` |
| `historia-cen.mjs` | Łowca, codziennie (woła go `feedy-lego.py`) | Dopisuje najniższą dzienną cenę zestawu do `src/data/historia-cen/RRRR-MM.jsonl` — **tylko zmiany**, format JSON Lines. Materiał na wykresy historii ceny; w `src/data`, bo runnery commitują ten katalog; buildowi nie ciąży, bo `.jsonl` nikt nie importuje. `--sucho` |
