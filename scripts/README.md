...
| `historia-cen.mjs` | Łowca, codziennie (woła go `feedy-lego.py`) | Dopisuje najniższą dzienną cenę zestawu do `materialy/historia-cen/RRRR-MM.jsonl` — **tylko zmiany**, format JSON Lines. Materiał na wykresy historii ceny; celowo poza `src/data`, żeby nie rosło w buildzie. `--sucho` |
